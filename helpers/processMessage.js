...
module.exports = async function processMessage(senderId, webhook_event) {
  let message = {
    text:
  ...
  let dob = "";
  let isNewUser = false;
  const doc = await Cache.findOne({ senderId }, (err, doc) => {});
  ...
  if (webhook_event.toLowerCase() !== "restart") {
    ...
    if (webhook_event.match(/^Name/i)) {
      ...
      constants.yes.includes(webhook_event.toLowerCase()) &&
      convoNumber > 1
    ) {
      convoNumber = 3;
      message = {
      ...
      constants.no.includes(webhook_event.toLowerCase()) &&
      convoNumber > 1
    ) {
      convoNumber = 3;
